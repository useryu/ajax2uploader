 ...
 */
class WebSocketFileTransferEvent{
	/**
	 ...
	 */
	constructor(source, file, loaded, total, startTime) {
		this.source = source;
		this.loaded = loaded;
		this.total = total;
...
class WebSocketFileTransfer{
	...
	initializeUpload() {
		...
		this.sendCommand('INIT: '+JSON.encode(infos));
	...
	sendAuth(login, password) {
		...
		this.sendCommand('AUTH: '+JSON.encode(infos));
